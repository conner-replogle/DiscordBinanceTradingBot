...
use chrono::{Date, DateTime, Duration, DurationRound, Utc, NaiveDateTime};
...
impl Schedule {
    ...
    pub fn open_time_slots(set_start_time: Option<DateTime<Utc>>,config: &Arc<Config>) -> Result<Vec<TimeSlot>, diesel::result::Error> {
        let interval = match config.get("schedule", "reservation_interval_min") {
            crate::config::ValueType::INT(Some(num)) => num,
            a => {
                warn!("reservation_interval_min did not have a value {a:#?}");
                15
            }
        };
        let reservations = Self::pull_reservations()?;
        let mut right_now = set_start_time.unwrap_or(Utc::now()
        ...
        while right_now < max_reservation {
            ...
            for reservation in reservations.iter(){
                if set_start_time.is_some(){
                    ...
                    if reservation.start_time <= right_now.naive_utc()  && right_now.naive_utc() < reservation.end_time{
            ...
            if let Some(reserved) =slot_reserved{
                possible_times.push(TimeSlot::RESERVED { reservation:reserved.clone()});
    ...
    pub fn create_reservation(
        ...
        for reservation in reservations.iter(){
            if reservation.start_time < new_reservation.end_time && reservation.end_time > new_reservation.start_time{
                return Ok(false);
            }
        }
